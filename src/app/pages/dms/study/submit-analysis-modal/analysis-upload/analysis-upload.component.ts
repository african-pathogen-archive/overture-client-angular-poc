...
export class AnalysisUploadComponent implements OnChanges {
  ...
  constructor(@SkipSelf() private http: HttpClient, private _service: SubmitService) {}
  ...
  uploadFiles() {
    ...
    this._service.submitUsingPOST(this.studyId, this.jsonPayLoad.toString() , `Bearer ${'2860ace0-9d90-478d-920d-a0a5fde1da8b'}`, "response", true)
      .subscribe(
        (event: any) => {
          // Upload Progress
          console.log(event);

          blobToText(event.body).subscribe((result200) => {
          ...
          if (event.type === HttpEventType.UploadProgress) {
            ...
            if (event.status === 200 || event.status === 201) {
              ...
              if (!isError) {
                this.uploadMsgText = `${this.replaceTexts.afterUploadMsg_success} : EventID = ${this.analysis.analysisId}`;
